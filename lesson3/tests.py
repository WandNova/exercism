...
class TestLesson3(unittest.TestCase):
    def test_task_1(self):
        ...
        for variant, (power_pellet_active, touching_ghost, result) in enumerate(
                zip(power_data, touching_data, result_data), start=1):
            with self.subTest(f'variation #{variant}', power_pellet_active=power_pellet_active,
                              touching_ghost=touching_ghost, result=result):
    ...
    def test_task_2(self):
        ...
        for variant, (touching_power_pellet, touching_dot, result) in enumerate(
                zip(touching_power_data, touching_dot_data, result_data), start=1):
            with self.subTest(f'variation #{variant}', touching_power_pellet=touching_power_pellet,
                              touching_dot=touching_dot, result=result):
    ...
    def test_task_4(self):
        ...
        for variant, (has_eaten_all_dots, power_pellet_active, touching_ghost, result) in enumerate(
                zip(all_dots_data, power_data, touching_data, result_data), start=1):
            with self.subTest(f'variation #{variant}', has_eaten_all_dots=has_eaten_all_dots,
                              power_pellet_active=power_pellet_active, touching_ghost=touching_ghost, result=result):
